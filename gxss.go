...
const (
	...
	// [NEW] Probe Resilience Configuration
	ProbeMaxRetries      = 3
...
func main() {
	...
	cookieRaw := flag.String("cookie", "", "Custom Cookies")
	...
	if debugMode {
		fmt.Printf("%s[*] DIAGNOSTIC MODE: ENABLED (Heartbeat Active)%s\n", NeonRed, Reset)
	}
				...
				if verifyDynamic(allocCtx, f.URL) {
					if _, hit := vulnerableParams.Load(pKey); !hit {
						...
						fmt.Printf("%s%s[!] XSS VALIDATED! %s[%s] %s| %-10s %s| %s%s%s\n",
							ClearLine, NeonGreen, Reset, f.Context, NeonYellow, f.Param, Reset, NeonCyan, f.URL, Reset)
	...
	for _, t := range targets {
		...
		if _, seen := wafCache.Load(host); !seen {
			...
			if wafName != "" {
				...
				fmt.Printf("%s%s[*] WAF Check: No common WAF detected on %s%s\n", ClearLine, NeonGreen, host, Reset)
			}
		}
		...
		for param, values := range originalQuery {
			...
			// B. Payload Selection (HYBRID CROSS-CONTEXT)
			...
			if smartMode {
				if payloads, ok := cachedPayloads[detectedContext]; ok {
					for _, p := range payloads {
						payloadsToRun = append(payloadsToRun, struct{Content string; CtxName string}{p, contextToString(detectedContext)})
					}
				}
				if detectedContext == HTML_CONTEXT {
					if payloads, ok := cachedPayloads[ATTRIBUTE_CONTEXT]; ok {
						for _, p := range payloads {
							payloadsToRun = append(payloadsToRun, struct{Content string; CtxName string}{p, "ATTR_X"})
						}
					}
				} else if detectedContext == ATTRIBUTE_CONTEXT {
					if payloads, ok := cachedPayloads[HTML_CONTEXT]; ok {
						for _, p := range payloads {
							payloadsToRun = append(payloadsToRun, struct{Content string; CtxName string}{p, "HTML_X"})
						}
					}
				}
				if detectedContext == HTML_CONTEXT || detectedContext == ATTRIBUTE_CONTEXT || detectedContext == URL_CONTEXT {
					if poly, ok := cachedPayloads[POLYGLOT_CONTEXT]; ok {
						for _, p := range poly {
							payloadsToRun = append(payloadsToRun, struct{Content string; CtxName string}{p, "POLY"})
						}
					}
				}
				if isUrlParam {
					for _, p := range urlParamsCache {
						payloadsToRun = append(payloadsToRun, struct{Content string; CtxName string}{p, "URL"})
					}
				}
				if waf, ok := cachedPayloads[WAF_BYPASS_CONTEXT]; ok {
					for _, p := range waf {
						payloadsToRun = append(payloadsToRun, struct{Content string; CtxName string}{p, "WAF"})
					}
				}

			} else {
				for _, p := range customPayloads {
					payloadsToRun = append(payloadsToRun, struct{Content string; CtxName string}{p, "CUSTOM"})
				}
			}
			...
			for _, item := range payloadsToRun {
				...
				go func(pl string, ctxName string, targetUrl url.URL, pName string, pOrig string, appendMode bool) {
					...
					if checkReflection(client, attackURL, pl) {
						browserChan <- Finding{
							URL:       attackURL,
...
func verifyDynamic(allocCtx context.Context, target string) bool {
	...
	tasks = append(tasks, 
		...
		network.SetBlockedURLs([]string{
			// HAPUS blokir gambar agar event onerror bisa jalan
			"*.css", "*.woff", "*.woff2", "*.ttf",
			"*.mp4", "*.webm", "*.mp3",
			"*.pdf", "*.zip", "*.iso",
...
func checkReflection(c *http.Client, target string, payload string) bool {
...
func checkChameleonProbe(c *http.Client, base string, param string, origVal string, smartProbe string) (bool, PayloadContext, bool) {
	...
	if ok, ctx := fireSimpleProbe(c, u.String(), "gxss"); ok {
		if debugMode {
			fmt.Printf("\n[DEBUG] Smart Probe (%s) Reflected on %s", smartProbe, param)
		}
	...
	if origVal != "" {
		...
		if ok, ctx := fireSimpleProbe(c, u.String(), "gxss"); ok {
			if debugMode {
				fmt.Printf("\n[DEBUG] Smart Append Reflected on %s", param)
			}
...
func fireSimpleProbe(c *http.Client, target string, keyword string) (bool, PayloadContext) {
	...
	for i := 0; i < ProbeMaxRetries; i++ {
		...
		// Jika request berhasil, proses body
		if err == nil {
			defer resp.Body.Close()
			...
			// Probe tidak ditemukan di body (Reflected = False)
			return false, 0
		}
		
		// Jika error (timeout/reset), tunggu sebentar lalu retry
		if debugMode {
			// fmt.Printf("\n[DEBUG] Probe Retry %d: %s", i+1, err)
		}
		time.Sleep(ProbeRetryDelay)
	}

	// Gagal total setelah 3x retry
	return false, 0
}

func contextToString(ctx PayloadContext) string {
	switch ctx {
	case HTML_CONTEXT: return "HTML"
	case ATTRIBUTE_CONTEXT: return "ATTR"
	case JAVASCRIPT_CONTEXT: return "JS"
	case URL_CONTEXT: return "URL"
	case POLYGLOT_CONTEXT: return "POLY"
	case WAF_BYPASS_CONTEXT: return "WAF"
	case JSONP_CONTEXT: return "JSONP"
	default: return "UNK"
	}
}
...
func readLines(path string) ([]string, error) {
	...
	for scanner.Scan() {
		...
		if text != "" { lines = append(lines, text) }
	}
...
func printBanner() {
 ...
 ╚██████╔╝ ██╔██╗ ███████║███████║     ██║      ██║  ██║██║██║ ╚═╝ ██║███████╗
  ╚═════╝  ╚═╝╚═╝ ╚══════╝╚══════╝     ╚═╝      ╚═╝  ╚═╝╚═╝╚═╝     ╚═╝╚══════╝` + Dim + `
       [ STATUS: GRANDMASTER V13.2 (RESILIENCE PATCH) ] [ ARCHITECT: REI ]` + Reset + "\n")
}

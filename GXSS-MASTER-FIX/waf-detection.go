...
func DetectWAF(targetURL string, client *http.Client) string {
	...
	// Gunakan UA dan Cookies global dari gxss.go (perlu akses variabel global atau inject)
	// Karena di sini terpisah package main, kita inject UA random manual jika tidak ada akses ke global
	// Namun karena package main sama, kita bisa akses userAgents jika public, tapi amannya kita hardcode atau pass UA.
	// Di gxss.go, getRandomUA() ada di package main, jadi bisa dipanggil.
	req.Header.Set("User-Agent", getRandomUA())
	req.Close = true

	// Akses globalCookies dari package main (karena package sama)
